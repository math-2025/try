...
import { PlusCircle, Users, Copy, Group, Trash2, UserX, UserCheck, KeyRound, Pencil } from "lucide-react";
  ...
  const handleRemoveStudent = async (studentId: string) => {
    ...
    try {
        ...
        toast({
            ...
            description: `${studentToRemove.name} adlı şagird sistemdən tamamilə silindi.`,
        });
    } catch (error) {
                  ...
                  {isLoading ? (
                    ...
                    <TableRow key={student.id} className={cn(student.status === 'disabled' && 'text-muted-foreground bg-muted/50')}>
                      ...
                      <TableCell>{student.email}</TableCell>
                       <TableCell>{student.status === 'active' ? 'Aktiv' : 'Deaktiv'}</TableCell>
                        ...
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant={student.status === 'active' ? 'outline' : 'secondary'} size="icon" className="h-8 w-8">
                              {student.status === 'active' ? <UserX /> : <UserCheck />}
                            </Button>
                          </AlertDialogTrigger>
                          <AlertDialogContent>
                            <AlertDialogHeader>
                ...
                <div className="space-y-4 py-4">
                    <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                           ...
                           <Button variant="outline" size="icon" onClick={() => copyToClipboard(newStudentCreds?.email || '')}><Copy /></Button>
                        </div>
                    </div>
                     <div className="space-y-1">
                        <Label>Şifrə</Label>
                        <div className="flex items-center gap-2">
                           ...
                           <Button variant="outline" size="icon" onClick={() => copyToClipboard(newStudentCreds?.pass || '')}><Copy /></Button>
                        </div>
                    </div>
                </div>
            </DialogContent>
                ...
                <div className="space-y-4 py-4">
                    <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                           ...
                           <Button variant="outline" size="icon" onClick={() => copyToClipboard(viewingCredsFor?.email || '')}><Copy /></Button>
                        </div>
                    </div>
                     <div className="space-y-1">
                        <Label>Şifrə</Label>
                        <div className="flex items-center gap-2">
                           ...
                           <Button variant="outline" size="icon" onClick={() => copyToClipboard(viewingCredsFor?.pass || '')}><Copy /></Button>
                        </div>
                    </div>
                </div>
            </DialogContent>
