...
export default function CheatersPage() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(submissionsQuery, async (snapshot) => {
            ...
            const cheaterDetails = await Promise.all(flaggedSubmissions.map(async (sub) => {
                let studentName = 'Bilinməyən Şagird';
                let studentGroup = 'N/A';
                
                if (sub.studentId) {
                    const studentQuery = query(collection(db, 'students'), where('id', '==', sub.studentId));
                ...
                let examTitle = 'Bilinməyən İmtahan';
                if(sub.examId) {
                    const examRef = doc(db, 'exams', sub.examId);
                    const examSnap = await getDocs(query(collection(db, 'exams'), where('id', '==', sub.examId)));
                     if(!examSnap.empty) {
                        examTitle = (examSnap.docs[0].data() as Exam).title;
                    }
                }
